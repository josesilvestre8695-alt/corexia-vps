...
<div class="card" style="max-width:620px">
 <h2 style="margin:0 0 6px">Autenticacao em 2 fatores (2FA) &mdash; Admin</h2>
 <p class="muted">Conta-mestra. Use um app autenticador (Google Authenticator, Authy, 1Password). Nao depende de telefone/WhatsApp.</p>
 <div id="sg_msg" class="msg"></div>
 <div id="sg_state" style="margin-top:8px">carregando...</div>
</div>
<script>
window.PAGE_INIT=sgLoad;
var _sec='';
function sgMsg(t,ok){ var m=$('sg_msg'); if(m){ m.textContent=t; m.className='msg '+(ok?'ok':'err'); if(ok)setTimeout(function(){m.className='msg';},3000); } }
function grp(s){ return (s||'').replace(/(.{4})/g,'$1 ').trim(); }
async function sgLoad(){ try{ var s=await api('GET','/api/auth/2fa/status'); sgRender(s); }catch(e){ $('sg_state').innerHTML='<span class="muted">erro: '+esc(e.message)+'</span>'; } }
function sgRender(s){
 if(s.enabled){
  $('sg_state').innerHTML='<div style="color:var(--ok);font-weight:600;margin-bottom:8px">&#9989; 2FA ATIVO ('+(s.method==="totp"?"App autenticador":"WhatsApp")+')</div>'
   +'<div class="muted" style="margin-bottom:12px">No proximo login vamos pedir o codigo de 6 digitos do app.</div>'
   +'<div class="fld"><label>Sua senha (para desativar)</label><input id="sg_pw" type="password" style="max-width:260px"></div>'
   +'<button class="act" style="color:var(--bad)" onclick="sgDisable()">Desativar 2FA</button>';
 } else {
  $('sg_state').innerHTML='<div class="muted" style="margin-bottom:10px">2FA desativado. Configure o app autenticador:</div>'
   +'<button class="btn-primary" onclick="sgStart()">Configurar app autenticador</button>'
   +'<div id="sg_setup" style="display:none;margin-top:16px;border-top:1px solid var(--border);padding-top:14px">'
   +'<ol class="muted" style="margin:0 0 12px;padding-left:18px;line-height:1.8">'
   +'<li>Abra o app autenticador no celular.</li>'
   +'<li>Toque em <b>&ldquo;Inserir chave de configuracao&rdquo;</b> (setup key).</li>'
   +'<li>Nome da conta: <b>Corexia</b> &nbsp;|&nbsp; Tipo: <b>baseado em tempo (TOTP)</b>.</li>'
   +'<li>Cole a chave abaixo e salve.</li></ol>'
   +'<div class="fld"><label>Chave (secret)</label><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><code id="sg_sec" style="font-size:16px;letter-spacing:2px;background:rgba(127,127,127,.14);padding:8px 12px;border-radius:8px"></code><button class="act" onclick="sgCopy()">Copiar</button></div></div>'
   +'<div class="fld" style="margin-top:10px"><label>Agora digite o codigo de 6 digitos que aparece no app</label><input id="sg_code" inputmode="numeric" maxlength="6" style="max-width:180px;letter-spacing:4px;font-size:18px"></div>'
   +'<button class="btn-primary" onclick="sgConfirm()">Confirmar e ativar</button></div>';
 }
}
async function sgStart(){ try{ var r=await api('POST','/api/auth/2fa/start',{method:'totp'}); _sec=r.secret||''; $('sg_setup').style.display='block'; $('sg_sec').textContent=grp(_sec); sgMsg('Chave gerada. Adicione no app e confirme abaixo.',true); }catch(e){ sgMsg('Erro: '+e.message,false); } }
function sgCopy(){ try{ navigator.clipboard.writeText(_sec); sgMsg('Chave copiada.',true); }catch(e){ sgMsg('Copie manualmente.',false); } }
async function sgConfirm(){ var code=($('sg_code').value||'').replace(/\D/g,''); if(code.length<6){ sgMsg('Digite os 6 digitos do app',false); return; } try{ await api('POST','/api/auth/2fa/confirm',{method:'totp',code:code}); sgMsg('2FA ativado com sucesso!',true); sgLoad(); }catch(e){ sgMsg('Codigo invalido. Use o codigo ATUAL que o app mostra.',false); } }
async function sgDisable(){ var pw=($('sg_pw')||{}).value||''; if(!pw){ sgMsg('Digite sua senha',false); return; } if(!confirm('Desativar o 2FA da conta admin?'))return; try{ await api('POST','/api/auth/2fa/disable',{password:pw}); sgMsg('2FA desativado.',true); sgLoad(); }catch(e){ sgMsg('Erro: '+e.message,false); } }
</script>
"""


@router.get("/comercial/seguranca")
def com_seguranca(req: Request):
    return _shell_react("Seguranca (2FA)", _ADMIN_SEG_BODY)


_PORTAL_SEG_PAGE = r"""<!doctype html><html lang="pt-br"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Seguranca</title>
<style>
 :root{--org:#f97316;--bg:#0b0f14;--card:#12161d;--ink:#eef2f6;--mut:#8b96a6;--bd:#232b35;--ok:#22c55e;--bad:#ef4444}
 *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--ink);font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif}
 .top{display:flex;align-items:center;gap:12px;padding:16px;border-bottom:1px solid var(--bd);position:sticky;top:0;background:var(--bg)}
 .back{color:var(--mut);text-decoration:none;font-size:24px;line-height:1}
 ...
 h2{margin:0 0 6px;font-size:18px} .mut{color:var(--mut);font-size:14px;line-height:1.6}
 input{width:100%;background:#0e131a;border:1px solid var(--bd);border-radius:10px;color:#fff;padding:12px;font-size:16px;margin-top:10px}
 .code{text-align:center;letter-spacing:8px;font-size:24px}
 .btn{display:block;width:100%;margin-top:14px;background:var(--org);color:#1a1205;border:0;border-radius:10px;padding:13px;font-weight:800;font-size:15px;cursor:pointer}
 ...
 .pill{display:inline-flex;align-items:center;gap:6px;background:rgba(34,197,94,.14);color:var(--ok);border-radius:999px;padding:6px 12px;font-weight:700;font-size:14px}
 .hide{display:none}
</style></head><body>
<div class="top"><a class="back" href="/portal">&#8592;</a><b>Seguranca da conta</b></div>
<div class="wrap"><div class="card">
 <h2>Verificacao em 2 passos</h2>
 <p class="mut">Alem da senha, pedimos um codigo no seu WhatsApp quando alguem tentar entrar na sua conta. Deixa sua conta muito mais segura.</p>
 <div id="msg" class="msg"></div>
 <div id="state">carregando...</div>
</div></div>
<script>
var TK=localStorage.getItem('corexia_token'); if(!TK){ location.href='/'; }
function $(id){return document.getElementById(id);}
function msg(t,ok){ var m=$('msg'); m.textContent=t||''; m.className='msg '+(ok?'ok':(t?'err':'')); if(ok)setTimeout(function(){m.className='msg';m.textContent='';},3500); }
async function api(method,path,body){
 var o={method:method,headers:{'Authorization':'Bearer '+TK}};
 if(body){ o.headers['Content-Type']='application/json'; o.body=JSON.stringify(body); }
 var r=await fetch(path,o); var d={}; try{ d=await r.json(); }catch(e){}
 if(!r.ok) throw new Error((d&&d.error)||('erro '+r.status)); return d;
}
async function load(){ try{ var s=await api('GET','/api/auth/2fa/status'); render(s); }catch(e){ $('state').innerHTML='<span class="mut">erro: '+e.message+'</span>'; } }
function render(s){
 if(s.enabled){
  $('state').innerHTML='<div class="pill">&#9989; Ativado</div>'
   +'<p class="mut" style="margin-top:12px">Sua conta esta protegida. No proximo login vamos pedir o codigo do WhatsApp'+(s.phone_mask?(' '+s.phone_mask):'')+'.</p>'
   +'<input id="pw" type="password" placeholder="Sua senha (para desativar)">'
   +'<button class="btn danger" onclick="off()">Desativar</button>';
 } else {
  $('state').innerHTML='<button class="btn" onclick="on()">Ativar por WhatsApp</button>'
   +'<div id="cf" class="hide" style="margin-top:14px;border-top:1px solid var(--bd);padding-top:14px">'
   +'<p class="mut">Enviamos um codigo no seu WhatsApp <b id="dest"></b>. Digite abaixo:</p>'
   +'<input id="code" class="code" inputmode="numeric" maxlength="6" placeholder="000000">'
   +'<button class="btn" onclick="conf()">Confirmar e ativar</button></div>';
 }
}
async function on(){ try{ var r=await api('POST','/api/auth/2fa/start',{method:'wa'}); $('dest').textContent=r.dest||''; $('cf').className=''; msg('Codigo enviado no seu WhatsApp.',true); }catch(e){ msg(e.message,false); } }
async function conf(){ var c=($('code').value||'').replace(/\D/g,''); if(c.length<6){ msg('Digite os 6 digitos',false); return; } try{ await api('POST','/api/auth/2fa/confirm',{method:'wa',code:c}); msg('Pronto! Verificacao em 2 passos ativada.',true); load(); }catch(e){ msg(e.message,false); } }
async function off(){ var pw=($('pw').value||''); if(!pw){ msg('Digite sua senha',false); return; } try{ await api('POST','/api/auth/2fa/disable',{password:pw}); msg('Desativado.',true); load(); }catch(e){ msg(e.message,false); } }
load();
</script></body></html>"""


@router.get("/portal/seguranca")
def portal_seguranca(req: Request):
    return HTMLResponse(_PORTAL_SEG_PAGE, headers={"Cache-Control": "no-cache"})
...
<div class="card" style="max-width:560px">
 <h2 style="margin:0 0 6px">Autenticacao em 2 fatores (2FA)</h2>
 <p class="muted">Um codigo no login, alem da senha. Protege sua conta e a de todos os seus clientes. Recomendado.</p>
 <div id="sg_msg" class="msg"></div>
 <div id="sg_state" style="margin-top:8px">carregando...</div>
</div>
<script>
...
function sgRender(s){
 if(s.enabled){
  $('sg_state').innerHTML='<div style="color:var(--ok);font-weight:600;margin-bottom:8px">&#9989; 2FA ATIVO ('+(s.method==="wa"?"WhatsApp":"App autenticador")+')</div>'
   +'<div class="muted" style="margin-bottom:12px">No proximo login vamos pedir o codigo.'+(s.phone_mask?(' O codigo vai para '+esc(s.phone_mask)+'.'):'')+'</div>'
   +'<div class="fld"><label>Sua senha (para desativar)</label><input id="sg_pw" type="password" style="max-width:260px"></div>'
   +'<button class="act" style="color:var(--bad)" onclick="sgDisable()">Desativar 2FA</button>';
 } else {
  $('sg_state').innerHTML='<div class="muted" style="margin-bottom:10px">2FA desativado nesta conta. Ative por WhatsApp:</div>'
   +'<button class="btn-primary" onclick="sgStart()">Ativar 2FA por WhatsApp</button>'
   +'<div id="sg_confirm" style="display:none;margin-top:14px;border-top:1px solid var(--border);padding-top:12px">'
   +'<div class="muted" style="margin-bottom:6px">Enviamos um codigo no seu WhatsApp <b id="sg_dest"></b>. Digite abaixo:</div>'
   +'<div class="fld"><label>Codigo (6 digitos)</label><input id="sg_code" inputmode="numeric" maxlength="6" style="max-width:180px;letter-spacing:4px;font-size:18px"></div>'
   +'<button class="btn-primary" onclick="sgConfirm()">Confirmar e ativar</button></div>';
 }
}
async function sgStart(){ try{ var r=await api('POST','/api/auth/2fa/start',{method:'wa'}); $('sg_dest').textContent=r.dest||''; $('sg_confirm').style.display='block'; sgMsg('Codigo enviado no seu WhatsApp.',true); }catch(e){ sgMsg('Erro: '+e.message,false); } }
async function sgConfirm(){ var code=($('sg_code').value||'').replace(/\D/g,''); if(code.length<6){ sgMsg('Digite os 6 digitos',false); return; } try{ await api('POST','/api/auth/2fa/confirm',{method:'wa',code:code}); sgMsg('2FA ativado!',true); sgLoad(); }catch(e){ sgMsg('Erro: '+e.message,false); } }
async function sgDisable(){ var pw=($('sg_pw')||{}).value||''; if(!pw){ sgMsg('Digite sua senha',false); return; } if(!confirm('Desativar o 2FA da sua conta?'))return; try{ await api('POST','/api/auth/2fa/disable',{password:pw}); sgMsg('2FA desativado.',true); sgLoad(); }catch(e){ sgMsg('Erro: '+e.message,false); } }
</script>
"""


@router.get("/provedor/seguranca")
def prov_seguranca_page(req: Request):
    return _prov_shell("seguranca", "Seguranca (2FA)", _PROV_SEG_BODY, req)
...
<div style="display:flex;gap:10px;align-items:center;margin-bottom:12px;flex-wrap:wrap">
 <b>Controle de Acesso - historico de rostos</b><span style="flex:1"></span>
 <select id="ac_filtro" onchange="acLoad()" style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;color:var(--ink);padding:8px 11px">
  <option value="">Todos</option><option value="acesso_facial">Reconhecidos</option><option value="facial_desconhecido">Desconhecidos</option></select>
 <button onclick="acLoad()">Atualizar</button></div>
<div id="msg" class="msg"></div>
<div class="cards" style="margin-bottom:12px">
 <div class="kpi"><div class="k">Reconhecidos (recentes)</div><div class="v" id="ac_kok" style="color:var(--ok)">-</div></div>
 <div class="kpi"><div class="k">Desconhecidos (recentes)</div><div class="v" id="ac_kno" style="color:var(--bad)">-</div></div></div>
<div id="ac_rows"><div class="center" style="color:var(--muted)">carregando...</div></div>
<script>
window.PAGE_INIT=acLoad;
function acDt(s){ s=(''+(s||'')); var d=s.slice(0,10).split('-'); var t=s.slice(11,16); return d.length===3?(d[2]+'/'+d[1]+' '+t):s; }
function acNome(desc){ desc=(''+(desc||'')).replace('[IA] ',''); var m=desc.match(/reconhecido:\\s*(.+)$/i); return m?m[1].trim():''; }
async function acLoad(){ try{
  var f=$('ac_filtro').value; var arr=[];
  if(f){ arr=await api('GET','/api/comercial/prov/alertas?limit=300&tipo='+f); }
  else { var a=await api('GET','/api/comercial/prov/alertas?limit=300&tipo=acesso_facial'); var b=await api('GET','/api/comercial/prov/alertas?limit=300&tipo=facial_desconhecido'); arr=a.concat(b); arr.sort(function(x,y){return (''+(y.criado||'')).localeCompare(''+(x.criado||''));}); }
  var nok=arr.filter(function(x){return x.tipo==='acesso_facial';}).length;
  var nno=arr.filter(function(x){return x.tipo==='facial_desconhecido';}).length;
  $('ac_kok').textContent=nok; $('ac_kno').textContent=nno;
  if(!arr.length){ $('ac_rows').innerHTML='<div class="center" style="color:var(--muted);padding:30px">Nenhum acesso registrado ainda. Ative o Controle de Acesso Facial numa camera de portaria e cadastre os moradores em Cameras e IA.</div>'; return; }
  var h='<table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="text-align:left;color:var(--muted)"><th style="padding:8px 6px">Data/hora</th><th style="padding:8px 6px">Camera</th><th style="padding:8px 6px">Resultado</th></tr></thead><tbody>';
  arr.forEach(function(x){ var ok=(x.tipo==='acesso_facial'); var nome=acNome(x.descricao);
   var res=ok?('<span style="color:var(--ok)">reconhecido: '+esc(nome||'-')+'</span>'):'<span style="color:var(--bad)">DESCONHECIDO</span>';
   h+='<tr style="border-top:1px solid var(--border)"><td style="padding:8px 6px;white-space:nowrap;color:var(--muted)">'+esc(acDt(x.criado))+'</td><td style="padding:8px 6px">'+esc(x.camera_nome||'-')+'</td><td style="padding:8px 6px">'+res+'</td></tr>'; });
  h+='</tbody></table>'; $('ac_rows').innerHTML=h;
 }catch(e){ msg('Erro: '+(e&&e.message?e.message:e)); } }
</script>
"""
...
def prov_generico(slug: str, req: Request):
    ...
    if slug == "acessos":
        return _prov_shell("acessos", "Controle de Acesso", _PROV_ACESSOS_BODY, req)
...
IA_MODULOS = [
    {"key": "corexia", "nome": "IA Corexia", "valor": 27.0, "pacote": True,
     "analiticos": [["arma_fogo", "Arma de fogo"], ["arma_branca", "Arma branca / faca"],
                    ...
                    ["pessoa", "Pessoa"], ["queda", "Pessoa caida / queda"], ["capacete", "Capacete / moto (rosto coberto)"], ["aglomeracao", "Aglomeracao de pessoas"], ["briga", "Briga / agressao (assistivo)"], ["crianca_elevador", "Crianca sozinha no elevador (Lei Miguel)"], ["crianca_sozinha", "Crianca sozinha na area (30s)"]]},
    ...
    {"key": "facial", "nome": "IA Controle de Acesso Facial", "valor": 47.0, "requer_entrada": True, "analiticos": [["facial", "Controle de acesso facial (portaria)"]]},
    {"key": "guarda_piscina", "nome": "IA Guarda-Piscina", "valor": 47.0, "requer_zona": True, "analiticos": [["guarda_piscina", "Guarda-Piscina (piscina vazia; armavel pelo cliente)"]]},
    {"key": "suspeito", "nome": "IA Detector de Suspeitos", "valor": 47.0, "pacote": True, "analiticos": [["suspeito", "Merodeio / permanencia (rondar, ficar parado)"], ["furto", "Ocultacao / furto (varejo - guardar produto)"]]},
]
...
# ==================== CONTROLE DE ACESSO FACIAL (cadastro on-prem, LGPD) ====================
# Fonte da verdade = VPS (entidade FaceEnroll: so o VETOR do rosto, nunca a foto). O cadastro e
# capturado PELA PROPRIA CAMERA por um worker na Xeon (requisito dos testes). Exclusao aqui apaga
# o dado biometrico (LGPD).
def _facial_wsecret_ok(val):
    import hmac as _h
    _s = os.getenv("WEBHOOK_SECRET", "")
    return bool(_s) and _h.compare_digest(str(val or ""), _s)


@router.post("/api/comercial/prov/facial/cadastrar")
async def prov_facial_cadastrar(req: Request):
    u, pid = _prov_req(req)
    if not pid:
        return JSONResponse({"error": "sem permissao"}, status_code=403)
    b = await req.json()
    ...
    if not cid or not _prov_camera_own(pid, cid):
        return JSONResponse({"error": "camera nao encontrada ou sem acesso"}, status_code=403)
    if not nome:
        return JSONResponse({"error": "informe o nome do morador"}, status_code=400)
    if not b.get("consent"):
        return JSONResponse({"error": "e obrigatorio o consentimento (LGPD - dado biometrico)"}, status_code=400)
    cam = _get_ent("Camera", cid) or {}
    rid = _create_ent("FaceEnrollReq", {
        "camera_id": cid, "camera_nome": cam.get("nome", ""), "stream_key": cam.get("stream_key", ""),
        "provedor_id": pid, "cliente_id": cam.get("cliente_id", ""), "nome": nome,
        "consent": True, "status": "pendente", "n": 0, "msg": ""})
    return {"success": True, "req_id": rid}


@router.get("/api/comercial/prov/facial/lista")
def prov_facial_lista(req: Request):
    u, pid = _prov_req(req)
    if not pid:
        return JSONResponse({"error": "sem permissao"}, status_code=403)
    cid = (req.query_params.get("camera_id") or "").strip()
    if not cid or not _prov_camera_own(pid, cid):
        return JSONResponse({"error": "sem acesso"}, status_code=403)
    c = _db()
    moradores, reqs = [], []
    for r in c.execute("SELECT id,data,created_date FROM entities WHERE entity='FaceEnroll'").fetchall():
        d = json.loads(r["data"])
        if d.get("camera_id") == cid:
            moradores.append({"id": r["id"], "nome": d.get("nome", ""), "n": len(d.get("embeddings") or []), "criado": r["created_date"]})
    for r in c.execute("SELECT id,data,created_date FROM entities WHERE entity='FaceEnrollReq'").fetchall():
        d = json.loads(r["data"])
        if d.get("camera_id") == cid and d.get("status") in ("pendente", "falhou"):
            reqs.append({"id": r["id"], "nome": d.get("nome", ""), "status": d.get("status"), "msg": d.get("msg", "")})
    c.close()
    moradores.sort(key=lambda x: (x["nome"] or "").lower())
    return {"moradores": moradores, "requisicoes": reqs}


@router.post("/api/comercial/prov/facial/excluir")
async def prov_facial_excluir(req: Request):
    u, pid = _prov_req(req)
    if not pid:
        return JSONResponse({"error": "sem permissao"}, status_code=403)
    b = await req.json()
    eid = (b.get("enroll_id") or "").strip()
    ent = _get_ent("FaceEnroll", eid)
    if not ent or ent.get("provedor_id") != pid:
        return JSONResponse({"error": "cadastro nao encontrado"}, status_code=403)
    c = _db(); c.execute("DELETE FROM entities WHERE entity='FaceEnroll' AND id=?", (eid,)); c.commit(); c.close()
    return {"success": True}


@router.post("/api/comercial/prov/facial/cancelar")
async def prov_facial_cancelar(req: Request):
    u, pid = _prov_req(req)
    if not pid:
        return JSONResponse({"error": "sem permissao"}, status_code=403)
    b = await req.json()
    rid = (b.get("req_id") or "").strip()
    ent = _get_ent("FaceEnrollReq", rid)
    if not ent or ent.get("provedor_id") != pid:
        return JSONResponse({"error": "requisicao nao encontrada"}, status_code=403)
    c = _db(); c.execute("DELETE FROM entities WHERE entity='FaceEnrollReq' AND id=?", (rid,)); c.commit(); c.close()
    return {"success": True}


@router.get("/api/facial/enroll-pending")
def facial_enroll_pending(req: Request):
    if not _facial_wsecret_ok(req.query_params.get("secret", "")):
        return JSONResponse({"error": "unauthorized"}, status_code=403)
    c = _db(); out = []
    for r in c.execute("SELECT id,data FROM entities WHERE entity='FaceEnrollReq'").fetchall():
        d = json.loads(r["data"])
        if d.get("status") == "pendente":
            out.append({"req_id": r["id"], "camera_id": d.get("camera_id"), "stream_key": d.get("stream_key", ""), "nome": d.get("nome", "")})
    c.close()
    return out


@router.post("/api/facial/enroll-done")
async def facial_enroll_done(req: Request):
    b = await req.json()
    if not _facial_wsecret_ok(b.get("secret")):
        return JSONResponse({"error": "unauthorized"}, status_code=403)
    rid = (b.get("req_id") or "").strip()
    embs = b.get("embeddings") or []
    n = int(b.get("n") or 0)
    msg = b.get("msg", "")
    r = _get_ent("FaceEnrollReq", rid)
    if not r:
        return JSONResponse({"error": "req nao existe"}, status_code=404)
    if n > 0 and embs:
        _create_ent("FaceEnroll", {"camera_id": r.get("camera_id"), "camera_nome": r.get("camera_nome", ""),
                                   "provedor_id": r.get("provedor_id"), "cliente_id": r.get("cliente_id", ""),
                                   "nome": r.get("nome"), "embeddings": embs, "consent": True})
        _update_ent("FaceEnrollReq", rid, {"status": "ok", "n": n, "msg": msg or ("cadastrado %d rosto(s)" % n)})
    else:
        _update_ent("FaceEnrollReq", rid, {"status": "falhou", "n": 0, "msg": msg or "nenhum rosto valido - aproxime o rosto e ilumine"})
    return {"success": True}


@router.get("/api/facial/roster")
def facial_roster(req: Request):
    if not _facial_wsecret_ok(req.query_params.get("secret", "")):
        return JSONResponse({"error": "unauthorized"}, status_code=403)
    c = _db(); out = {}
    for r in c.execute("SELECT data FROM entities WHERE entity='FaceEnroll'").fetchall():
        d = json.loads(r["data"]); cid = d.get("camera_id")
        if not cid:
            continue
        out.setdefault(cid, {}).setdefault(d.get("nome", "?"), []).extend(d.get("embeddings") or [])
    c.close()
    return out
...
<div class="ov" id="ov"><div class="modal" style="max-width:640px"><h2>IA - <span id="a_nome"></span></h2><input type="hidden" id="a_id">
 ...
 <div style="color:var(--muted);font-size:12px;margin-bottom:8px">Sem agenda = roda 24h. Crie janelas p/ restringir: escolha os analiticos + dias + horario. Ex.: "pessoa" so das 19h as 6h.</div>
 <div id="ags"></div>
 <button class="act" type="button" onclick="addAg()" style="margin-top:2px">+ Adicionar agenda</button>
 <div id="schedbox" style="display:none">
...
<div class="ov" id="ovz"><div class="modal" style="max-width:820px"><h2>Zonas - <span id="z_nome"></span></h2><input type="hidden" id="z_id">
 <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap">
  <button id="z_mzona" class="btn-primary" onclick="zMode('zona')">Zona (poligono)</button>
  <button id="z_mlinha" onclick="zMode('linha')">Linha</button>
  <button id="z_mheat" onclick="zMode('heatmap')">Area (mapa calor)</button>
  <button id="z_magua" onclick="zMode('agua')">Agua (piscina)</button>
  <button id="z_mvig" onclick="zMode('vigilancia')">Vigilancia (suspeitos)</button>
  <button onclick="zNova()">Finalizar forma</button>
  <button onclick="zUndo()">Desfazer ponto</button>
  <button onclick="zClear()" style="color:var(--bad)">Limpar tudo</button></div>
...
<div class="ov" id="ovfc"><div class="modal" style="max-width:520px"><h2>Rostos - <span id="fc_nome"></span></h2><input type="hidden" id="fc_id">
 <p style="color:var(--muted);font-size:13px;margin:0 0 10px">Cadastre os moradores <b>pela propria camera de portaria</b> (rosto proximo e iluminado). Guardamos so o padrao do rosto (vetor), nunca a foto. Requer consentimento (LGPD).</p>
 <div id="fc_list" style="margin-bottom:12px;max-height:240px;overflow:auto"><div class="center" style="color:var(--muted)">carregando...</div></div>
 <div style="border-top:1px solid var(--border);padding-top:10px">
  <div class="fld"><label>Nome do morador</label><input id="fc_new_nome" placeholder="Ex.: Joao - Ap 101"></div>
  <label class="ck" style="margin:2px 0 8px"><input type="checkbox" id="fc_consent" style="width:auto"> <span style="font-size:12px">Confirmo o consentimento do titular para uso do reconhecimento facial (LGPD)</span></label>
  <button class="btn-primary" onclick="facialCad()">Cadastrar pela camera agora</button>
  <div id="fc_msg" style="font-size:12px;color:var(--muted);margin-top:8px"></div>
 </div>
 <div class="foot"><button onclick="document.getElementById('ovfc').classList.remove('open')">Fechar</button></div></div></div>
...
function render(){ var q=($('q').value||'').toLowerCase(); var MN=modNome();
 ...
 $('grid').innerHTML=arr.map(function(c){ var mods=modsAtivos(c.config); var on=c.config&&c.config.ativo&&mods.length>0;
  ...
  var busca=''; if(DATA.gravacao==='cloud'){ if(c.busca_ia){ busca='<button class="act" style="color:var(--ok)" data-id="'+c.id+'" data-on="1" onclick="toggleBusca(this)">Pergunte: ON</button>'; } else if((c.dias_gravacao||0)>0){ busca='<button class="act" style="color:var(--accent)" data-id="'+c.id+'" data-on="0" onclick="toggleBusca(this)">Pergunte: ativar</button>'; } else { busca='<span class="act" style="color:var(--muted)" title="Ative a gravacao em nuvem primeiro">Pergunte (grave 1o)</span>'; } }
   ...
   '<div style="display:flex;gap:6px;flex-wrap:wrap"><button class="act" style="color:var(--accent)" onclick="ver(\\''+c.id+'\\')">ver ao vivo</button><button class="act" onclick="editarCam(\\''+c.id+'\\')">editar</button><button class="act" onclick="conf(\\''+c.id+'\\')">configurar IA</button><button class="act" onclick="zonas(\\''+c.id+'\\')">zonas</button><button class="act" onclick="editLocal(\\''+c.id+'\\')">local</button>'+busca+grav+del+'</div></div></div>'; }).join(''); }
...
function fecharLive(){ $('ovlive').classList.remove('open'); $('lv_frame').src='about:blank'; }
function toggleSched(){ $('schedbox').style.display=$('a_sched').checked?'block':'none'; }
var FC_CAM=null, FC_POLL=null;
function facialAbrir(){ var id=$('a_id').value; var c=CAMS.filter(function(x){return x.id===id})[0]; if(!c)return; FC_CAM=c;
 $('fc_id').value=id; $('fc_nome').textContent=c.nome||''; $('fc_new_nome').value=''; $('fc_consent').checked=false; $('fc_msg').textContent='';
 var lst=$('fc_list'); if(lst && !lst._bound){ lst._bound=1; lst.addEventListener('click',function(ev){ var t=ev.target; var d=t.getAttribute&&t.getAttribute('data-fdel'); var k=t.getAttribute&&t.getAttribute('data-fcancel'); if(d){facialDel(d);} else if(k){facialCancela(k);} }); }
 $('ovfc').classList.add('open'); facialLista(); }
async function facialLista(){ if(!FC_CAM)return; try{ var d=await api('GET','/api/comercial/prov/facial/lista?camera_id='+encodeURIComponent(FC_CAM.id));
  var h=''; (d.moradores||[]).forEach(function(m){ h+='<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--border)"><span>'+esc(m.nome)+' <span style="color:var(--muted);font-size:11px">('+m.n+' amostras)</span></span><button class="act" style="color:var(--bad)" data-fdel="'+m.id+'">excluir</button></div>'; });
  (d.requisicoes||[]).forEach(function(r){ var cor=(r.status==='falhou')?'var(--bad)':'var(--muted)'; h+='<div style="padding:6px 0;color:'+cor+';font-size:12px">'+esc(r.nome)+' &mdash; '+(r.status==='pendente'?'capturando pela camera...':('falhou: '+esc(r.msg||'')))+' <a href="#" data-fcancel="'+r.id+'" style="color:var(--muted)">remover</a></div>'; });
  $('fc_list').innerHTML=h||'<div style="color:var(--muted)">Nenhum morador cadastrado ainda.</div>';
  var pend=(d.requisicoes||[]).some(function(r){return r.status==='pendente';});
  if(pend && !FC_POLL){ FC_POLL=setInterval(facialLista,5000); } else if(!pend && FC_POLL){ clearInterval(FC_POLL); FC_POLL=null; }
 }catch(e){ $('fc_list').innerHTML='<div style="color:var(--bad)">Erro: '+(e&&e.message?e.message:e)+'</div>'; } }
async function facialCad(){ if(!FC_CAM)return; var nome=$('fc_new_nome').value.trim(); if(!nome){ $('fc_msg').textContent='Informe o nome.'; return; }
 if(!$('fc_consent').checked){ $('fc_msg').textContent='Marque o consentimento (LGPD).'; return; }
 $('fc_msg').textContent='Enviando... aponte o rosto do morador para a camera, de perto e iluminado.';
 try{ await api('POST','/api/comercial/prov/facial/cadastrar',{camera_id:FC_CAM.id,nome:nome,consent:true});
  $('fc_new_nome').value=''; $('fc_consent').checked=false; $('fc_msg').textContent='Capturando pela camera (ate ~1 min)...'; facialLista();
 }catch(e){ $('fc_msg').textContent='Erro: '+(e&&e.message?e.message:e); } }
async function facialDel(id){ if(!confirm('Excluir este rosto? (LGPD - remove o dado biometrico)'))return;
...
function onSub(k){ var mod=DATA.modulos.filter(function(m){return m.key===k})[0]; if(!mod)return;
 var _ce=$('d_crianca_elevador'); if(_ce&&_ce.checked&&!_ce._ok){ if(confirm('Esta camera e a do ELEVADOR? Ligue este analitico (Lei Miguel) SOMENTE na camera de dentro do elevador.')){ _ce._ok=true; } else { _ce.checked=false; } }
...
function selAnaliticos(){ var out=[]; (DATA.modulos||[]).forEach(function(mod){ var mc=$('m_'+mod.key); if(!mc||!mc.checked)return;
  ...
  else { mod.analiticos.forEach(function(a){out.push(a[0]);}); } }); return out; }
var _AGDIAS=[[1,'Seg'],[2,'Ter'],[3,'Qua'],[4,'Qui'],[5,'Sex'],[6,'Sab'],[7,'Dom']];
function agRow(preset){ preset=preset||{}; var keys=selAnaliticos().slice();
  (preset.analiticos||[]).forEach(function(k){ if(keys.indexOf(k)<0)keys.push(k); });
  var w=document.createElement('div'); w.className='agrow'; w.style.cssText='border:1px solid var(--border);border-radius:9px;padding:8px;margin-bottom:8px';
  var chips=keys.map(function(k){ var on=(preset.analiticos||[]).indexOf(k)>=0; return '<label class="ck" style="margin-right:8px"><input type="checkbox" class="agan" value="'+k+'"'+(on?' checked':'')+'> '+esc(_anaLabel(k))+'</label>'; }).join('') || '<span style="color:var(--muted);font-size:12px">marque os modulos de IA acima primeiro</span>';
  var dias=_AGDIAS.map(function(d){ var on=(preset.dias||[1,2,3,4,5,6,7]).indexOf(d[0])>=0; return '<label class="ck" style="margin-right:6px"><input type="checkbox" class="agd" value="'+d[0]+'"'+(on?' checked':'')+'> '+d[1]+'</label>'; }).join('');
  w.innerHTML='<div style="font-size:11px;color:var(--muted);margin-bottom:3px">Analiticos desta janela</div><div style="display:flex;flex-wrap:wrap;margin-bottom:6px">'+chips+'</div>'
   +'<div style="display:flex;flex-wrap:wrap;margin-bottom:6px">'+dias+'</div>'
   +'<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><span style="font-size:12px">Das</span><input type="time" class="agi" value="'+(preset.hora_inicio||'19:00')+'"><span style="font-size:12px">as</span><input type="time" class="agf" value="'+(preset.hora_fim||'06:00')+'"><span style="flex:1"></span><button class="act" type="button" style="color:var(--bad)" onclick="this.parentNode.parentNode.remove()">remover</button></div>';
  $('ags').appendChild(w); }
function addAg(){ agRow({}); }
function readAgs(){ var out=[]; Array.prototype.forEach.call(document.querySelectorAll('#ags .agrow'),function(w){
  var ana=[]; Array.prototype.forEach.call(w.querySelectorAll('.agan:checked'),function(e){ana.push(e.value);});
  var dias=[]; Array.prototype.forEach.call(w.querySelectorAll('.agd:checked'),function(e){dias.push(parseInt(e.value,10));});
  if(!ana.length)return; if(!dias.length)dias=[1,2,3,4,5,6,7];
  out.push({label:'Agenda',analiticos:ana,dias:dias,hora_inicio:(w.querySelector('.agi').value||'00:00'),hora_fim:(w.querySelector('.agf').value||'23:59')}); }); return out; }
...
/* ---- editor de zonas / area (canvas sobre o snapshot) - auto-atendido pelo provedor ---- */
var Z_SHAPES=[], Z_CUR=[], Z_MODE='zona', Z_CAM=null, Z_IMG=null;
...
function zonas(id){ var c=CAMS.filter(function(x){return String(x.id)===String(id)})[0]; if(!c)return; Z_CAM=c;
 ...
 fetch('/camthumb/'+id+'?t='+encodeURIComponent(TOKEN),{headers:{'Authorization':'Bearer '+TOKEN}}).then(function(r){ if(!r.ok)throw 0; return r.blob(); })
  .then(function(b){ var im=new Image(); im.onload=function(){ Z_IMG=im; var cv=$('z_cv');
    var w=Math.min(im.naturalWidth||780,780); cv.width=w; cv.height=Math.round(w*((im.naturalHeight||439)/(im.naturalWidth||780))); zDraw(); };
    im.src=URL.createObjectURL(b); })
  .catch(function(){ Z_IMG=null; zDraw(); msg('Sem foto da camera (offline) - da p/ desenhar mesmo assim.',true); });
}
function zDraw(){ var cv=$('z_cv'); var ctx=cv.getContext('2d'); ctx.clearRect(0,0,cv.width,cv.height);
 if(Z_IMG){ try{ctx.drawImage(Z_IMG,0,0,cv.width,cv.height);}catch(e){} } else { ctx.fillStyle='#111'; ctx.fillRect(0,0,cv.width,cv.height); ctx.fillStyle='#888'; ctx.fillText('(sem imagem)',16,24); }
 function poly(pts,close,color){ if(!pts.length)return; ctx.strokeStyle=color; ctx.fillStyle=color; ctx.lineWidth=2; ctx.beginPath();
...
function zClear(){ Z_SHAPES=[]; Z_CUR=[]; zDraw(); }
function zFecha(){ $('ovz').classList.remove('open'); }
function zEnsure(body){ var hz=Z_SHAPES.some(function(z){return z.tipo==='zona'}), hl=Z_SHAPES.some(function(z){return z.tipo==='linha'}), hh=Z_SHAPES.some(function(z){return z.tipo==='heatmap'});
 function add(a,v){ if(a.indexOf(v)<0)a.push(v); }
 function ens(a){ if(hz)add(a,'intruso'); if(hl)add(a,'linha'); if(hh)add(a,'heatmap'); }
 if(body.horarios&&body.horarios.length){ body.horarios.forEach(function(h){ h.analiticos=h.analiticos||[]; ens(h.analiticos); }); }
 else { body.analiticos_padrao=body.analiticos_padrao||[]; ens(body.analiticos_padrao); } }
async function zSalvar(){ var c=Z_CAM; if(!c)return; var poly=(Z_MODE!=='linha');
 if(poly&&Z_CUR.length>=3){ Z_SHAPES.push({tipo:Z_MODE,nome:(Z_MODE==='heatmap'?'Area ':(Z_MODE==='agua'?'Agua ':'Zona '))+(Z_SHAPES.length+1),pontos:Z_CUR.slice()}); Z_CUR=[]; }
 ...
 var body={ camera_id:c.id, camera_nome:c.nome||'', ativo:(cfg.ativo!==undefined?cfg.ativo:true),
  ...
  analiticos_padrao:(cfg.analiticos_padrao||[]).slice(), zonas_intrusao:Z_SHAPES };
 zEnsure(body);
 try{ await api('POST','/api/comercial/prov/analiticos/salvar',body); zFecha(); msg('Zonas salvas. Vale em ~2 min.',true); reload(); }catch(e){ msg('Erro ao salvar zonas: '+(e&&e.message||e)); } }
</script>
"""
